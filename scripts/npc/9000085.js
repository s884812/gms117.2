...
function action(mode, type, selection) {
	...
	if (mode != 1) {
		...
		if (status == 0) {
			...
			var selStr = "看到怪物: #r#e" + cm.getPlayer().getMonsterBook().getSeen() + "#n#k 种, 获得卡片: #b#e" + cm.getPlayer().getMonsterBook().getCaught() + "#n#k 种\r\n你想看哪个?\r\n\r\n#b";
			...
			if (selection == 0) {
				byName = true;
				cm.sendGetText("请在这里输入怪物名称（不能输入中文直接无视吧）");
				return;
			}
			selection--;
			...
			if (cm.getPlayer().getMonsterBook().getLevelByCard(theEntry.id) == 0) {
				cm.sendOk("#" + cm.getLeftPadded("" + (selection + 1), '0', 3) + " - 没有数据.");
				cm.safeDispose();
				return;
			}
			sel = selection;
			...
			info += "等级 " + theEntry.dummyBattler.getLevel() + "\r\n";
			info += "HP " + theEntry.dummyBattler.calcHP() + "\r\n";
			info += "经验值 " + theEntry.dummyBattler.getStats().getExp() + ", Pokemon EXP " + theEntry.dummyBattler.getOurExp() + "\r\n";
			info += "攻击力: " + theEntry.dummyBattler.getATK(0) +  ", 防御力: " + theEntry.dummyBattler.getDEF() + "%\r\n";
			info += "Sp.ATK: " + theEntry.dummyBattler.getSpATK(0) +  ", Sp.DEF: " + theEntry.dummyBattler.getSpDEF() + "%\r\n";
			info += "速度: " + theEntry.dummyBattler.getSpeed() +  ", 回避率: " + theEntry.dummyBattler.getEVA() + ", 命中率: " + theEntry.dummyBattler.getACC() + "\r\n";
			...
			if (theEntry.maps == null || theEntry.maps.size() <= 0) {
				...
				for (var xx = 0; xx < theEntry.maps.size(); xx++) {
					...
					info += "#m" + pr.left + "# - " + (pr.right / 100) + "% 几率\r\n";
				}
			}
			info += "\r\n#b\r\n";
			info += "#L0#查看这个怪物的暴率#l\r\n";
