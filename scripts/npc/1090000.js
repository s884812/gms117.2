...
function action(mode, type, selection) {
	...
	if (cm.getJob() == 0) {
		...
		cm.sendNext("你想成为 #r海盗#k ?");
	    } else {
		cm.sendOk("我可以让冒险家成为 #r海盗#k.");
		cm.dispose();
	    }
	} else {
	...
	cm.sendNextPrev("你的选择真是太正确了,海盗是非常强大的职业.");
    } else if (status == 2) {
	cm.sendYesNo("你真的想成为 #r海盗#k ?");
    } else if (status == 3) {
	if (cm.getJob() == 0) {
	    cm.resetStats(4, 4, 4, 4);
	    cm.expandInventory(1, 4);
	    cm.expandInventory(4, 4);
	    cm.changeJob(500); // WARRIOR
	    ...
	    cm.c.getPlayer().setRemainingSp((cm.getPlayer().getLevel() - 10) * 3 + 7);
