...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    private final void giveExpToCharacter(final MapleCharacter attacker, int exp, final boolean highestDamage, final int numExpSharers, final byte pty, final byte Class_Bonus_EXP_PERCENT, final byte Premium_Bonus_EXP_PERCENT, final int lastskillID) {
        ...
        if (getId() == 9300346) {
            attacker.changeMap(104000004,1);
        }
        ...
        if (exp > 0) {
            ...
            exp = (int) Math.min(Integer.MAX_VALUE, exp  * attacker.getEXPMod() * attacker.getStat().expBuff / 100.0 * (attacker.getLevel() < 1 ? GameConstants.getExpRate_Below10(attacker.getJob()) : GameConstants.getExpRate(attacker.getJob(), ChannelServer.getInstance(map.getChannel()).getExpRate()) + exp1));
	    ...
	    attacker.getTrait(MapleTraitType.charisma).addExp(stats.getCharismaEXP(), attacker);
            if (attacker.getParty() != null){
                if (attacker.partyMembersInMap() > 1) {
                    exp *= attacker.partyMembersInMap(); 
                }
            }
        ...
        attacker.modifyCSPoints(2, getLevel() * (int) Math.floor(Math.random()*1+ 1 ));
    }
