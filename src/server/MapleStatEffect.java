...
import static server.MapleStatEffect.calculateBoundingBox;
...
//    private List<Pair<Integer, Integer>> randomMorph;
    ...
    private static MapleStatEffect loadFromData(final MapleData source, final int sourceid, final boolean skill, final boolean overTime, final int level, final String variables) {
        ...
        if (ret.skill) { // hack because we can't get from the datafile...
            switch (sourceid) {
                ...
                case 5110011:
                    ret.statups.put(MapleBuffStat.CRITICAL_RATE_BUFF, ret.info.get(MapleStatInfo.x));
                    ret.statups.put(MapleBuffStat.MIN_CRITICAL_DAMAGE, ret.info.get(MapleStatInfo.x));
                    ret.statups.put(MapleBuffStat.MIN_CRITICAL_DAMAGE, ret.info.get(MapleStatInfo.x));
                    break;
                case 5221018:
                    ret.statups.put(MapleBuffStat.WATK, ret.info.get(MapleStatInfo.x));
                    ret.statups.put(MapleBuffStat.ABNORMAL_STATUS_R, ret.info.get(MapleStatInfo.x));
                    ret.statups.put(MapleBuffStat.ELEMENTAL_STATUS_R, ret.info.get(MapleStatInfo.x));
    ...
    public final void handleExtraPVP(MapleCharacter applyfrom, MapleCharacter chr) {
        if (sourceid == 2311005 || sourceid == 5121005 || sourceid == 1201006 || (GameConstants.isBeginnerJob(sourceid / 10000) && sourceid % 10000 == 104)) { //doom, threaten, snatch
            ...
            chr.registerEffect(this, starttime, BuffTimer.getInstance().schedule(new MapleStatEffect.CancelEffectAction(chr, this, starttime, localstatups), isSubTime(sourceid) ? getSubTime() : getDuration()), localstatups, false, getDuration(), applyfrom.getId());
        }
    }
    ...
    public final void silentApplyBuff(final MapleCharacter chr, final long starttime, final int localDuration, final Map<MapleBuffStat, Integer> statup, final int cid) {
        chr.registerEffect(this, starttime, BuffTimer.getInstance().schedule(new MapleStatEffect.CancelEffectAction(chr, this, starttime, statup),
...
//	final ScheduledFuture<?> schedule = TimerManager.getInstance().schedule(cancelAction, ((starttime + 99999) - System.currentTimeMillis()));
    ...
    public final void applyEnergyBuff(final MapleCharacter applyto, final boolean infinity, int targets) {
        ...
        if (infinity) {
            ...
            final MapleStatEffect.CancelEffectAction cancelAction = new MapleStatEffect.CancelEffectAction(applyto, this, starttime, stat);
