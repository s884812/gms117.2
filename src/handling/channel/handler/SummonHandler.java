...
public class SummonHandler {
    ...
    if ((sse != null) && (numAttacked > sse.mobCount)) {
      chr.dropMessage(5, "[警告] 请不要使用非法程序。");
    ...
    for (int i = 0; i < numAttacked; i++) {
      ...
      if (chr.isGM()) {
        //chr.dropMessage(-5, "召唤兽攻击 打怪伤害: " + damge + " 攻击次数: " + numAttacked);
      }
      if ((damge > 1599999) && (!chr.isGM())) {
        World.Broadcast.broadcastGMMessage(CWvsContext.serverNotice(6, "[GM Message] " + chr.getName() + " (等级 " + chr.getLevel() + ") 召唤兽攻击伤害异常。打怪伤害: " + damge + " 地图ID: " + chr.getMapId()));
      }
