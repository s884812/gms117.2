...
public class InventoryHandler {
    ...
    public static final void UseMagnify(final LittleEndianAccessor slea, final MapleClient c, MapleCharacter chr) {
        c.getPlayer().updateTick(slea.readInt());
        c.getPlayer().setScrolledPosition((short) 0);
                    ...
                    while (!rewarded) {
                        ...
                        if (pot != null && pot.reqLevel / 10 <= reqLevel && GameConstants.optionTypeFits(pot.optionType, eqq.getItemId()) && GameConstants.potentialIDFits(pot.opID, new_state, i)) { //optionType
                            ...
                            if (i == 0) {
                                ...
                                //System.out.println("setPotential4 " + pot.opID);
                            } else if (i == 1) {
                                eqq.setPotential2(pot.opID);
                                //System.out.println("setPotential2 " + pot.opID);
                            } else if (i == 2) {
                                eqq.setPotential3(pot.opID);
                                //System.out.println("setPotential3 " + pot.opID);
                            } else if (i == 3) {
                                eqq.setPotential1(pot.opID);
                                //System.out.println("setPotential1 " + pot.opID);
                            } else if (i == 4) {
                                eqq.setPotential5(pot.opID);
                                //System.out.println("setPotential5 " + pot.opID);
                            }
                            rewarded = true;
                        }
                    }
    ...
    public static final boolean UseUpgradeScroll(final short slot, final short dst, final short ws, final MapleClient c, final MapleCharacter chr, final int vegas, final boolean legendarySpirit) {
        ...
        if (scrollSuccess == Equip.ScrollResult.CURSE) {
            if (dst < 0) {
                if (chr.haveItem(5064000) && toScroll.getEnhance() <= 12) {
                    c.getPlayer().dropMessage(5, "由于拥有防爆卷轴，你的装备没有消失.");
                ...
                if (chr.haveItem(5064000) && toScroll.getEnhance() <= 12) {
                    c.getPlayer().dropMessage(5, "由于拥有防爆卷轴，你的装备没有消失，你的装备没有消失.");
                    c.getSession().write(InventoryPacket.scrolledItem(scroll, toScroll, false, false));
                    wocao = 1;
                }  else {
        ...
        if (dst < 0 && (scrollSuccess == Equip.ScrollResult.SUCCESS || scrollSuccess == Equip.ScrollResult.CURSE) && vegas == 0) {
            if (chr.haveItem(5064000) && toScroll.getEnhance() <= 12) {
                chr.equipChanged();
                MapleInventoryManipulator.removeById(c, MapleInventoryType.CASH, 5064000, 1, false, false);
                if (wocao == 0) {
                c.getPlayer().dropMessage(5, "由于拥有防爆卷轴，防爆卷轴失去了一个.");
                } else {
                }
                }  else {
            chr.equipChanged();
        ...
        if (toUse != null && toUse.getQuantity() >= 1 && toUse.getItemId() == itemId && !chr.hasBlockedInventory() && !chr.inPVP()) {
            switch (toUse.getItemId()) {
                ...
                case 2430112: //miracle cube fragment
                    if (c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) { //判断背包是否有空位
                        if (c.getPlayer().getInventory(MapleInventoryType.USE).countById(2430112) >= 25) { //判断是否有25个物品.
                ...
                case 2430481: //super miracle cube fragment
                    if (c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
                        if (c.getPlayer().getInventory(MapleInventoryType.USE).countById(2430481) >= 2) {
                            if (MapleInventoryManipulator.checkSpace(c, 5062005, 1, "") && MapleInventoryManipulator.removeById(c, MapleInventoryType.USE, toUse.getItemId(), 2, true, false)) {
                                MapleInventoryManipulator.addById(c, 5062005, (short) 1, "Scripted item: " + toUse.getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());
                            } else {
                                c.getPlayer().dropMessage(5, "背包满了.");
                            }
                ...
                case 2430016:
                case 2430051:
                    if (MapleInventoryManipulator.checkSpace(c, 5220040, (short) 1, "")) {
                        int shu = Randomizer.nextInt(5) + 1;
                        MapleInventoryManipulator.addById(c, 5220040, (short) shu, "Reward item: " + toUse.getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());
                        MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, slot, (byte) 1, false);
                        c.getPlayer().message("您从中获得了" + shu + "个转蛋卷.");
                    }
                    break;
                ...
                case 2430191: //可爱新手礼包           
             if (c.getPlayer().getGender() == 0) {
                MapleInventoryManipulator.addById(c, 1040154, (short) 1, "Reward item: " + toUse.getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());                   
                MapleInventoryManipulator.addById(c, 1003192, (short) 1, "Reward item: " + toUse.getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());
                MapleInventoryManipulator.addById(c, 1060145, (short) 1, "Reward item: " + toUse.getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());
             } 
             else if(c.getPlayer().getGender() == 1){
                MapleInventoryManipulator.addById(c, 1041156, (short) 1, "Reward item: " + toUse.getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());                      
                MapleInventoryManipulator.addById(c, 1003193, (short) 1, "Reward item: " + toUse.getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());
                MapleInventoryManipulator.addById(c, 1061166, (short) 1, "Reward item: " + toUse.getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());
             }                                  
                MapleInventoryManipulator.addById(c, 1142263, (short) 1, "Reward item: " + toUse.getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());
                MapleInventoryManipulator.addById(c, 3700012, (short) 1, "Reward item: " + toUse.getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());                
                MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, slot, (byte) 1, false);
                  break;  
                case 2430192: //快点长大礼包                                 
                MapleInventoryManipulator.addById(c, 5152056, (short) 1, "Reward item: " + toUse.getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());
                MapleInventoryManipulator.addById(c, 5150053, (short) 1, "Reward item: " + toUse.getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());                               
                MapleInventoryManipulator.addById(c, 1142264, (short) 1, "Reward item: " + toUse.getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());                
                MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, slot, (byte) 1, false);
                  break; 
             case 2430193: //豪华玩家礼包           
             if (c.getPlayer().getGender() == 0) {
                MapleInventoryManipulator.addById(c, 1050186, (short) 1, "Reward item: " + toUse.getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());                   
             } 
             else if(c.getPlayer().getGender() == 1){
                MapleInventoryManipulator.addById(c, 1051227, (short) 1, "Reward item: " + toUse.getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());                      
             }                
                MapleInventoryManipulator.addById(c, 1003194, (short) 1, "Reward item: " + toUse.getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());
                MapleInventoryManipulator.addById(c, 1142265, (short) 1, "Reward item: " + toUse.getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());               
                MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, slot, (byte) 1, false);
                  break;
                default:
                    System.out.println("× ——  物品/未定义‖编号(" + toUse.getItemId() + ")");
                    break;
            }
        }
        if (mountid > 0) {
    ...
    public static final void UseCashItem(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (itemId) {
            ...
            case 5050000: { // AP Reset
                ...
                switch (apto) { // AP to
                    case 64: // str
                        if (playerst.getStr() >= 32767) {
                            used = false;
                        }
                        break;
                    case 128: // dex
                        if (playerst.getDex() >= 32767) {
                            used = false;
                        }
                        break;
                    case 256: // int
                        if (playerst.getInt() >= 32767) {
                            used = false;
                        }
                        break;
                    case 512: // luk
                        if (playerst.getLuk() >= 32767) {
                            used = false;
                        }
                        break;
                    case 2048: // hp
            ...
            case 5534000: { //tims lab
                ...
                if (item != null) {
                    ...
                    if (eq.getState() == 0) {
                        ...
                        c.getPlayer().dropMessage(5, "无法清洗.");
                    }
                } else {
            ...
            case 5062000: { //神奇魔方
                if (c.getPlayer().getLevel() < 1) {
                    c.getPlayer().dropMessage(1, "至少需要 1 级才能使用.");
                } else {
                    final Item item = c.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem((byte) slea.readInt());
                    if (item != null && c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
                        final Equip eq = (Equip) item;
                        int stateRate = c.getPlayer().getClient().getChannelServer().getStateRate();
            ...
            case 5050100: { // 
                ...
                c.getPlayer().setRemainingAp((c.getPlayer().getLevel() - 1) * 5 + 8);
