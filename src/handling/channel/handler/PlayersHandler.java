...
import client.*;
import static client.MapleCharacter.FameStatus.NOT_THIS_MONTH;
import static client.MapleCharacter.FameStatus.NOT_TODAY;
import static client.MapleCharacter.FameStatus.OK;
...
import static handling.channel.handler.PlayersHandler.DoRing;
import static handling.channel.handler.PlayersHandler.inArea;
        ...
        if (!area) {
            ...
            for (MapleCharacter attacked : chr.getMap().getCharactersIntersect(box)) {
                if (attacked.getId() != chr.getId() && attacked.isAlive() && !attacked.isHidden() && (type == 0 || attacked.getTeam() != chr.getTeam())) {
                    ...
                    } else if (chr.getJob() == 412 || chr.getJob() == 422 || chr.getJob() == 434 || chr.getJob() == 1411 || chr.getJob() == 1412 || chr.getJob() == 432) {
                        int[] skills = {4120005, 4220005, 4340001, 14110004, 4320005};
                        ThreadLock.lock();
                        try {
                            for (int i : skills) {
