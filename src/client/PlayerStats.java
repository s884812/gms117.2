...
import static client.PlayerStats.pvpSkills;
import constants.GameConstants;
import client.inventory.MapleInventoryType;
import client.inventory.Item;
import client.inventory.Equip;
import client.inventory.MapleWeaponType;
import static client.inventory.MapleWeaponType.AXE1H;
import static client.inventory.MapleWeaponType.AXE2H;
import static client.inventory.MapleWeaponType.BLUNT1H;
import static client.inventory.MapleWeaponType.BLUNT2H;
import static client.inventory.MapleWeaponType.BOW;
import static client.inventory.MapleWeaponType.CANE;
import static client.inventory.MapleWeaponType.CANNON;
import static client.inventory.MapleWeaponType.CLAW;
import static client.inventory.MapleWeaponType.CROSSBOW;
import static client.inventory.MapleWeaponType.DAGGER;
import static client.inventory.MapleWeaponType.DUAL_BOW;
import static client.inventory.MapleWeaponType.GUN;
import static client.inventory.MapleWeaponType.KATARA;
import static client.inventory.MapleWeaponType.KNUCKLE;
import static client.inventory.MapleWeaponType.POLE_ARM;
import static client.inventory.MapleWeaponType.SPEAR;
import static client.inventory.MapleWeaponType.STAFF;
import static client.inventory.MapleWeaponType.SWORD1H;
import static client.inventory.MapleWeaponType.SWORD2H;
import static client.inventory.MapleWeaponType.WAND;
...
public class PlayerStats implements Serializable {
    ...
    public void recalcLocalStats(boolean first_login, MapleCharacter chra) {
        ...
        localmaxhp_ += Math.floor((percent_hp * localmaxhp_) / 100.0f + add_hp);
        localmaxhp_ += chra.getTrait(MapleTraitType.will).getLevel() * 20;
        localmaxhp = Math.min(99999, Math.abs(Math.max(-99999, localmaxhp_)));

        localmaxmp_ += Math.floor((percent_mp * localmaxmp_) / 100.0f + add_mp);
    ...
    private void handlePassiveSkills(MapleCharacter chra) {
        ...
        switch (chra.getJob()) {
            ...
            case 434: { // Blade Master
                bx = SkillFactory.getSkill(4330008); // 永恒黑暗
                    bof = chra.getTotalSkillLevel(bx);
                    if (bof > 0) {
                        eff = bx.getEffect(bof);
                        percent_hp += eff.getPercentHP();
