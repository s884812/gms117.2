  ...
  public static class warpoxtop extends CommandExecute
  {
    ...
    public static class AP extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            c.getPlayer().updateSingleStat(MapleStat.AVAILABLEAP, c.getPlayer().getRemainingAp()); // we don't care the value here
            return 1;
        }
    }
    ...
    public static class qianneng extends CommandExecute {
        @Override
        public int execute(MapleClient c, String[] splitted) {
            Equip equip = (Equip) c.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem((short) 1);
            switch (splitted.length) {
                case 4:
                    equip.setPotential1(Integer.parseInt(splitted[1]));
                    equip.setPotential2(Integer.parseInt(splitted[2]));
                    equip.setPotential3(Integer.parseInt(splitted[3]));
                    c.getPlayer().forceReAddItem(equip, MapleInventoryType.EQUIP);
                    break;
                case 3:
                    switch (Integer.parseInt(splitted[1])) {
                        case 1:
                            equip.setPotential1(Integer.parseInt(splitted[1]));
                            break;
                        case 2:
                            equip.setPotential2(Integer.parseInt(splitted[1]));
                            break;
                        case 3:
                            equip.setPotential3(Integer.parseInt(splitted[1]));
                            break;
                    }
                    c.getPlayer().forceReAddItem(equip, MapleInventoryType.EQUIP);
                    break;
                default:
                    c.getPlayer().dropMessage(6, "Legendary Code");
                    c.getPlayer().dropMessage(6, "Unique Code");
                    break;
            }
    ...
    public static class Level extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            if (c.getPlayer().getLevel() < Short.parseShort(splitted[1])) {
            while (c.getPlayer().getLevel() < Short.parseShort(splitted[1])) {
            ...
            if (c.getPlayer().getExp() < 0) {
                ...
                c.getPlayer().dropMessage(6, "你现在的等级是 " + c.getPlayer().getLevel() + ".");
            }
            return 1;
        }
    }
