...
public class PortalScriptManager {
    ...
    private final PortalScript getPortalScript(final String scriptName) {
        ...
        try {
            System.out.println("· ——  穿越时空？‖行动代号："+scriptName+".js‖此事定有蹊跷。");
            fr = new FileReader(scriptFile);
            CompiledScript compiled = ((Compilable) portal).compile(fr);
            compiled.eval();
        } catch (final Exception e) {
            System.err.println("× ——  传送/出错‖编号(" + scriptName + ")" + e);
            FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "× ——  传送/出错‖编号(" + scriptName + ")" + e);
        } finally {
            if (fr != null) {
                try {
                    fr.close();
    ...
    public final void executePortalScript(final MaplePortal portal, final MapleClient c) {
        ...
        if (script != null) {
            try {
                ...
                System.err.println("× ——  传送/出错‖编号(" + portal.getScriptName() + ")" + e);
            }
        } else {
           System.out.println("· ——  传送点/未定义‖编号(" + portal.getScriptName() + ") ");
           c.getPlayer().dropMessage(-1,"传送点失灵？     行动代号：" + portal.getScriptName() + "     哦麦噶的！");
        }
    }
