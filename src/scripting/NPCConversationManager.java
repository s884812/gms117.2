        ...
        if (ranks != null && ranks.size() > 0) {
            ...
            for (int i = 0; i < ranks.size(); i++) {
                ...
                if (de.chance > 0 && (de.questid <= 0 || (de.questid > 0 && MapleQuest.getInstance(de.questid).getName().length() > 0))) {
                    itemId = de.itemId;
                    if (num == 0) {
                        name.append("察看 #o" + mobId + "# 的暴率\r\n");
                        name.append("--------------------------------------\r\n");
                    }
                    String namez = "#z" + itemId + "#";
                    if (itemId == 0) { //meso
                        itemId = 4031041; //display sack of cash
                        namez = (de.Minimum * getClient().getChannelServer().getMesoRate()) + " to " + (de.Maximum * getClient().getChannelServer().getMesoRate()) + " 金币";
                    }
                    ch = de.chance * getClient().getChannelServer().getDropRate();
                    name.append((num + 1) + ") #v" + itemId + "#" + namez + " - " + (Integer.valueOf(ch >= 999999 ? 1000000 : ch).doubleValue() / 10000.0) + "% 的几率. " + (de.questid > 0 && MapleQuest.getInstance(de.questid).getName().length() > 0 ? ("需要这个物品的任务是 " + MapleQuest.getInstance(de.questid).getName() + " .") : "") + "\r\n");
                    num++;
                }
            }
            if (name.length() > 0) {
